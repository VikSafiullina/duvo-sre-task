...
key="smoke-$(date +%s)-$$"
first=$(curl -fsS -X POST "$BASE_URL/sandboxes" -H 'content-type: application/json' -H "Idempotency-Key: $key" -d '{"type":"http"}' | jq -r .sandbox_id)
again=$(curl -fsS -X POST "$BASE_URL/sandboxes" -H 'content-type: application/json' -H "Idempotency-Key: $key" -d '{"type":"http"}' | jq -r .sandbox_id)
