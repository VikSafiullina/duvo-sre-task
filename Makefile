...
logs: ## Tail api + worker logs
	$(COMPOSE) logs -f --tail=50 api worker
...
chaos: ## Restart worker with 30% injected job failures (make chaos RATE=0 to stop)
	CHAOS_FAILURE_RATE=$(or $(RATE),0.3) $(COMPOSE) up -d --wait worker
...
urls: ## Print useful local URLs
	...
	@echo "Worker      http://localhost:9100/metrics"
