...
def test_create_routes_job_to_the_weighted_pool(client: TestClient, weight: int, pool: str) -> None:
    ...
    assert f'queue_depth{{deployment="{pool}"}} 1.0' in body
    assert f'queue_depth{{deployment="{other}"}} 0.0' in body
    enqueued = f'jobs_enqueued_total{{deployment="{pool}",job="start_sandbox",outcome="enqueued"}}'
